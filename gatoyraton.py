...
def enter_move(board):
    ...
    while not ok:
        ...
        if sign in ['O', 'X']:
            ...
            ok = True
            board[row][col] = 'O'  # colocar 'O' en el cuadro seleccionado
...
def draw_move(board):
    free = make_list_of_free_fields(board)  # crear una lista de los cuadros vacios o libres
    cnt = len(free)
    if cnt > 0:  # si la lista no está vacía, elegir un lugar para 'X' y colocarla
        this = randrange(cnt)
        row, col = free[this]
        board[row][col] = 'X'
